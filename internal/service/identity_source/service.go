...
func (s *service) GetAuthURL(ctx context.Context, providerType string) (string, error) {
	...
	source, err := s.findEnabledOIDC(ctx, providerType)
	if err != nil {
...
// findEnabledOIDC 查找指定 provider_type 的启用 OIDC 身份源
func (s *service) findEnabledOIDC(ctx context.Context, providerType string) (domain.IdentitySource, error) {
	sources, err := s.repo.List(ctx)
	if err != nil {
		return domain.IdentitySource{}, err
	}
	...
	for _, src := range sources {
		if src.Enabled && string(src.OIDCConfig.ProviderType) == providerType {
			return src, nil
		}
	}

	return domain.IdentitySource{}, fmt.Errorf("未找到类型为 %s 的启用 OIDC 身份源", providerType)
}
